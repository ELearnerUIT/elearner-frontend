import { convertTitleToLink } from "@/app/(public)/courses/[courseName]/page";
    ...
    return (
        <div className="bg-white rounded-lg p-4 flex gap-4 shadow-sm">
            ...
            <div className="flex-1">
                <Link href={`/courses/${convertTitleToLink(courseTitle)}`} className="font-semibold text-gray-900 mb-1">
                    {courseTitle}
                </Link>
            ...
            <div className="text-right flex flex-col justify-between">
                <div>
                    <div className="text-xl font-bold text-indigo-600">${price.toLocaleString()}</div>
                    <div className="text-sm text-gray-400 line-through">${oldPrice.toLocaleString()}</div>
                </div>
                <button className="text-red-500 hover:text-red-700 flex items-center gap-1 text-sm" onClick={() => onRemove(id)}>
                    <Trash size={20} />
                    Remove
                </button>
            </div>
