...
const get_color_class = (color: ButtonColor, enabled: boolean) => {
    ...
    const default_color_class =
        "bg-[rgba(99,102,241,1)] hover:bg-[rgba(80,71,217,1)] active:bg-[rgba(60,60,194,1)] border-[rgba(99,102,241,1)] text-white";
    if (color_info) {
        return (
            (enabled
                ? color_info.bg_normal +
                  " " +
                  color_info.bg_hover +
                  " " +
                  color_info.bg_active
                : color_info.bg_disable) +
            " " +
            color_info.border +
            " " +
            color_info.text
...
export function CustomLinkButton({
    ...
    return (
        <Link
            href={href}
            className={`${width} ${height} border ${color_class} font-normal py-2 px-4 ${rounded} flex justify-center items-center gap-2 transition`}
        >
            {text}
            {children}
        </Link>
