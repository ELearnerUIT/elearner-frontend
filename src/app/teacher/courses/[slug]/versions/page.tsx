...
export default function CourseVersionsPage() {
    ...
    const [dateFilter, setDateFilter] = useState<"all" | "createdAt" | "publishedAt">("all");
    ...
    const filteredVersions = useMemo(() => {
        ...
        if (dateFilter !== "all" && (startDate || endDate)) {
            filtered = filtered.filter((v) => {
                ...
                if (dateFilter === "createdAt") {
                    targetDate = v.createdAt;
                } else if (dateFilter === "publishedAt") {
                    targetDate = v.publishedAt;
                }

                if (!targetDate) return false;

                const versionDate = new Date(targetDate);
                ...
                if (start && end) {
                    ...
                    return versionDate >= start;
                } else if (end) {
                    return versionDate <= end;
                }
        ...
        return filtered;
    }, [versions, searchQuery, dateFilter, startDate, endDate]);
    ...
    return (
        <CourseManagementLayout course={course}>
            <div className="space-y-6">
                ...
                {/* Search & Filter */}
                <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-lg border border-slate-200 dark:border-slate-700 space-y-4">
                    {/* Search Bar */}
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                            ...
                            <select
                                ...
                                className="px-4 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:text-white text-sm"
                            >
                            ...
                            <div className="relative">
                                <input
                                    ...
                                    className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:text-white text-sm disabled:opacity-50 disabled:cursor-not-allowed"
                        ...
                        {(dateFilter !== "all" || searchQuery) && (
                            <div className="flex flex-wrap gap-2 pt-2">
                                {searchQuery && (
                                    <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-indigo-100 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 rounded-lg text-xs font-medium">
                                        Search: {searchQuery}
                                        <button
                                ...
                                {dateFilter !== "all" && (
                                    <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 rounded-lg text-xs font-medium">
                                        {dateFilter === "createdAt" ? "Created" : "Published"}
                                ...
                                <button
                                    ...
                                    className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-600 rounded-lg text-xs font-medium transition-colors"
                                >
                                    Clear All
                                </button>
                            </div>
